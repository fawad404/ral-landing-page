...
export default function CustomSelect({ value, onChange, options, placeholder, hasError }: CustomSelectProps) {
  ...
  return (
    <div ref={ref} className="relative w-full">
      <button
        ...
        className={`w-full flex items-center justify-between gap-2 px-4 py-3 bg-white border rounded-xl text-sm transition-colors focus:outline-none ${
          hasError
            ...
            : 'border-border hover:border-[#09488B40]'
        }`}
      >
      ...
      {open && (
        <div className="absolute z-50 top-full mt-1 left-0 w-full bg-white border border-border rounded-xl shadow-lg py-1 max-h-56 overflow-y-auto">
          {options.map((opt) => (
            <button
              key={opt.value}
              type="button"
